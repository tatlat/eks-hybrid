...
# only uploading nodeadm, ATTRIBUTION.txt, and GIT_VERSION to latest/ (skipping ginkgo/e2e-test/nodeadm.test)
# Skip the latest/ upload if SKIP_LATEST_UPLOAD is set
...
# Use the SAME _bin/ directory structure that build stage creates so all buildspecs work without modification
# Artifact key is configurable for different regions (e.g., artifacts-cn.zip for China)
