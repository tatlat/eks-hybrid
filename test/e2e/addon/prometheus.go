...
func (p *PrometheusNodeExporterTest) Run(ctx context.Context) error {
	p.addon = Addon{
		Cluster:   p.Cluster,
...
func (p *PrometheusNodeExporterTest) Create(ctx context.Context) error {
	...
	// Node exporter typically runs as a DaemonSet to collect metrics from each node
	if err := kubernetes.WaitForDaemonSetReady(ctx, p.Logger, p.K8S, p.addon.Namespace, p.addon.Name); err != nil {
		return err
	}
...
func (p *PrometheusNodeExporterTest) Validate(ctx context.Context) error {
	...
	svc, err := p.K8S.CoreV1().Services(prometheusNamespace).Get(ctx, prometheusName, metav1.GetOptions{})
	if err != nil {
...
func (p *PrometheusNodeExporterTest) checkNodeExporterMetrics(ctx context.Context, metricsEndpoint string) error {
	...
	return wait.ExponentialBackoffWithContext(ctx, retryBackoff, func(ctx context.Context) (bool, error) {
		...
		return p.checkForExpectedMetrics(string(body)), nil
	})
}
...
func (p *PrometheusNodeExporterTest) CollectLogs(ctx context.Context) error {
