...
func (p *PrometheusNodeExporterTest) Validate(ctx context.Context) error {
	...
	svc, err := ik8s.GetRetry(ctx, p.K8S.CoreV1().Services(prometheusNamespace), prometheusName, nil)
	if err != nil {
...
func (p *PrometheusNodeExporterTest) PrintLogs(ctx context.Context) error {
	logs, err := kubernetes.FetchLogs(ctx, p.K8S, p.addon.Name, p.addon.Namespace, nil)
	if err != nil {
