	...
	When("hybrid and cloud-managed nodes coexist", func() {
		...
		Context("Pod-to-Pod Communication", func() {
			It("should enable cross-VPC pod-to-pod communication from hybrid to cloud nodes", func(ctx context.Context) {
				...
				err := kubernetes.CreateNginxPodInNode(ctx, test.K8sClient.Interface, cloudNodeName, testNamespace, test.Cluster.Region, test.Logger, test.DNSSuffix, test.EcrAccount, "nginx-cloud", testCaseLabels)
				...
				err = kubernetes.CreateNginxPodInNode(ctx, test.K8sClient.Interface, hybridNodeName, testNamespace, test.Cluster.Region, test.Logger, test.DNSSuffix, test.EcrAccount, "test-client-hybrid", testCaseLabels)
			...
			It("should enable cross-VPC pod-to-pod communication from cloud to hybrid nodes", func(ctx context.Context) {
				...
				err := kubernetes.CreateNginxPodInNode(ctx, test.K8sClient.Interface, hybridNodeName, testNamespace, test.Cluster.Region, test.Logger, test.DNSSuffix, test.EcrAccount, "nginx-hybrid-reverse", testCaseLabels)
				...
				err = kubernetes.CreateNginxPodInNode(ctx, test.K8sClient.Interface, cloudNodeName, testNamespace, test.Cluster.Region, test.Logger, test.DNSSuffix, test.EcrAccount, "test-client-cloud", testCaseLabels)
		...
		Context("Cross-VPC Service Discovery", func() {
			It("should enable cross-VPC service discovery from hybrid to cloud services", func(ctx context.Context) {
				...
				// Create deployment
				_, err := kubernetes.CreateDeployment(ctx, test.K8sClient.Interface, "nginx-service-cloud", testNamespace, test.Cluster.Region, cloudNodeSelector, httpPort, 1, test.Logger, test.DNSSuffix, test.EcrAccount, testCaseLabels)
				...
				err = kubernetes.CreateNginxPodInNode(ctx, test.K8sClient.Interface, hybridNodeName, testNamespace, test.Cluster.Region, test.Logger, test.DNSSuffix, test.EcrAccount, "test-client-hybrid-service", testCaseLabels)
			...
			It("should enable cross-VPC service discovery from cloud to hybrid services", func(ctx context.Context) {
				...
				_, err := kubernetes.CreateDeployment(ctx, test.K8sClient.Interface, "nginx-service-hybrid", testNamespace, test.Cluster.Region, hybridNodeSelector, httpPort, 1, test.Logger, test.DNSSuffix, test.EcrAccount, testCaseLabels)
				Expect(err).NotTo(HaveOccurred(), "creating deployment")
				...
				err = kubernetes.CreateNginxPodInNode(ctx, test.K8sClient.Interface, cloudNodeName, testNamespace, test.Cluster.Region, test.Logger, test.DNSSuffix, test.EcrAccount, "test-client-cloud-bidirectional", testCaseLabels)
				Expect(err).NotTo(HaveOccurred(), "creating client pod")
