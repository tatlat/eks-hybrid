	...
	When("using peered VPC", func() {
		...
		When("using ec2 instance as hybrid nodes", func() {
			It("runs node monitoring agent tests", func(ctx context.Context) {
				nodeMonitoringAgent := test.NewNodeMonitoringAgentTest()
				test.Logger.Info("Running test for node monitoring agent")

				DeferCleanup(func(ctx context.Context) {
					report := CurrentSpecReport()
					if report.State.Is(types.SpecStateFailed) {
						err := nodeMonitoringAgent.CollectLogs(ctx)
						if err != nil {
							// continue cleanup even if logs collection fails
							GinkgoWriter.Printf("Failed to collect node monitoring agent logs: %v\n", err)
						}
					}
					Expect(nodeMonitoringAgent.Delete(ctx)).To(Succeed(), "should cleanup node monitoring agent successfully")
				})

				Expect(nodeMonitoringAgent.Run(ctx)).To(
					Succeed(), "node monitoring agent test should have run successfully",
				)
